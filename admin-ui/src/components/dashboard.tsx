...
const retryModeLabels: Record<RetryMode, string> = {
  turbo: "Turbo",
  fast: "Fast",
  balanced: "Balanced",
  steady: "Steady",
...
const retryModeDescriptions: Record<RetryMode, string> = {
  turbo: "短冷却，高重试",
  fast: "默认快速切换",
  balanced: "均衡退避",
  steady: "尊重 Retry-After",
  polite: "保守低频",
  custom: "自定义参数",
};

const defaultCustomRetryPolicy: RetryPolicy = {
...
function RetryPolicyPanel() {
  ...
  useEffect(() => {
    ...
    } else if (data?.mode === "custom" && data.effectivePolicy) {
      setCustomPolicy(data.effectivePolicy);
    }
  }, [data?.customPolicy, data?.effectivePolicy, data?.mode]);

  const activeMode = data?.mode ?? "fast";
  const effective = data?.effectivePolicy;
  ...
  const applyMode = (mode: RetryMode, policy = customPolicy) => {
    setRetryPolicy.mutate(
      { mode, customPolicy: mode === "custom" ? policy : null },
      {
        onSuccess: (res) =>
          toast.success(`429 重试策略已切换到 ${retryModeLabels[res.mode]}`),
        onError: (err) => toast.error("设置失败: " + extractErrorMessage(err)),
      },
    );
  };

  const updateCustomNumber = (key: keyof RetryPolicy, value: string) => {
          ...
          <div className="grid w-full gap-3 sm:grid-cols-2 xl:max-w-3xl xl:grid-cols-3">
            <label className="text-xs font-medium text-muted-foreground">
              ...
              <Input
                ...
                min={500}
                max={120000}
                value={customPolicy.rateLimitCooldownMs}
                onChange={(event) =>
                  updateCustomNumber("rateLimitCooldownMs", event.target.value)
                }
                className="mt-1 h-8"
              />
            </label>
            <label className="text-xs font-medium text-muted-foreground">
              ...
              <Input
                ...
                min={1}
                max={30}
                value={customPolicy.maxRequestRetries}
                onChange={(event) =>
                  updateCustomNumber("maxRequestRetries", event.target.value)
                }
                className="mt-1 h-8"
              />
            </label>
            <label className="text-xs font-medium text-muted-foreground">
              ...
              <Input
                ...
                min={50}
                max={30000}
                value={customPolicy.baseBackoffMs}
                onChange={(event) =>
                  updateCustomNumber("baseBackoffMs", event.target.value)
                }
                className="mt-1 h-8"
              />
            </label>
            <label className="text-xs font-medium text-muted-foreground">
              ...
              <Input
                type="number"
                min={customPolicy.baseBackoffMs}
                max={120000}
                value={customPolicy.maxBackoffMs}
                onChange={(event) =>
                  updateCustomNumber("maxBackoffMs", event.target.value)
                }
                className="mt-1 h-8"
              />
            </label>
            <label className="flex min-h-8 items-center justify-between gap-3 rounded-md border border-border/60 px-3 py-2 text-xs font-medium text-muted-foreground">
              429 换凭据
              <Switch
                checked={customPolicy.credentialSwitchOn429}
                onCheckedChange={(checked) =>
                  setCustomPolicy((prev) => ({
                    ...prev,
                    credentialSwitchOn429: checked,
                  }))
                }
              />
            </label>
            <label className="flex min-h-8 items-center justify-between gap-3 rounded-md border border-border/60 px-3 py-2 text-xs font-medium text-muted-foreground">
              Retry-After
              <Switch
                checked={customPolicy.respectRetryAfter}
                onCheckedChange={(checked) =>
                  setCustomPolicy((prev) => ({
                    ...prev,
                    respectRetryAfter: checked,
                  }))
                }
              />
            </label>
            <Button
              size="sm"
              onClick={() => applyMode("custom", customPolicy)}
              disabled={setRetryPolicy.isPending}
