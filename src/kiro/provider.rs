...
impl KiroProvider {
    ...
    /// 热更新全局代理配置，并重建受保护的全局 client 缓存条目。
    pub fn update_global_proxy(&self, proxy: Option<ProxyConfig>) -> anyhow::Result<()> {
        let initial_client = build_client(proxy.as_ref(), 720, self.tls_backend)?;
        *self.global_proxy.write() = proxy.clone();
        *self.client_cache.lock() = ClientCache::new(proxy, initial_client, CLIENT_CACHE_CAP);
        tracing::info!("全局代理配置已热更新，HTTP client 缓存已重建");
        Ok(())
    }
    ...
    async fn call_mcp_with_retry(&self, request_body: &str) -> anyhow::Result<reqwest::Response> {
        ...
        let (_, retry_policy) = self.effective_retry_policy()?;
        let max_retries = Self::max_retries(total_credentials, &retry_policy);
        ...
        for attempt in 0..max_retries {
            ...
            let ctx = match self.token_manager.acquire_context(None).await {
                ...
                Err(e) => {
                    last_error = Some(e);
                    if let Some(delay) = self.token_manager.next_rate_limit_delay()
                        && attempt + 1 < max_retries
                    {
                        sleep(delay.min(Self::retry_delay(attempt, &retry_policy))).await;
                    }
                    continue;
                }
            };
            ...
            if status.as_u16() == 429 {
                ...
                let has_other = if retry_policy.credential_switch_on_429 {
                ...
                if attempt + 1 < max_retries {
                    if !has_other {
                        let delay = retry_after
                            .unwrap_or_else(|| Self::retry_delay(attempt, &retry_policy));
                        sleep(delay).await;
                    }
                    continue;
                }
    ...
    async fn call_api_with_retry(
        ...
        for attempt in 0..max_retries {
            ...
            if status.as_u16() == 429 {
                ...
                tracing::warn!(
                    "API 请求失败（普通 429 限流，凭据 #{}, 冷却 {:?}, 切换={}, 尝试 {}/{}）: {} {}",
                    ctx.id,
                    cooldown,
                    has_other,
                    attempt + 1,
    ...
    fn effective_retry_policy(
        &self,
    ) -> anyhow::Result<(crate::model::config::RetryMode, RetryPolicy)> {
        let (mode, _, effective) = self.token_manager.get_retry_policy()?;
        Ok((mode, effective))
    }

    fn max_retries(total_credentials: usize, policy: &RetryPolicy) -> usize {
        (total_credentials.max(1) * policy.max_request_retries).min(MAX_TOTAL_RETRIES)
    }
    ...
    fn retry_delay(attempt: usize, policy: &RetryPolicy) -> Duration {
