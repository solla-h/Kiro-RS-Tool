...
/// 设置凭据 endpoint；null 或空字符串表示回退到 defaultEndpoint。
...
/// 获取当前默认端点与兼容模式
pub async fn get_runtime_compatibility(State(state): State<AdminState>) -> impl IntoResponse {
    Json(state.service.get_runtime_compatibility())
}
...
pub async fn update_global_config(
    ...
    match state.service.update_global_config(payload) {
        Ok(response) => Json(response).into_response(),
        Err(e) => (e.status_code(), Json(e.into_response())).into_response(),
    }
}
